        ...
        lifecycleScope.launch {
            ...
            if (result.isSuccessful) {
                result.body()?.let {
                    ...
                    if(amILeader){
                        binding.rvGroupRegistrationList.layoutManager = LinearLayoutManager(requireContext())
                        activityViewModel.token?.let { token ->
                            val result = crewService.getRequestList(makeHeaderByAccessToken(token.accessToken), crew.crewId)
                            if(result.isSuccessful){
                                groupDetailFirstTabMemberRegisterListAdapter = GroupDetailFirstTabRegistrationListAdapter()
                                binding.rvGroupRegistrationList.adapter = groupDetailFirstTabMemberRegisterListAdapter.apply {
