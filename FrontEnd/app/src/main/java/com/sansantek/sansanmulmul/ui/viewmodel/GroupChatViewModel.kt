...
class ChatViewModel : ViewModel() {
    ...
    private var _chatList: MutableLiveData<MutableList<MessageHistory>> =
    ...
    fun runStomp() {
        ...
        stompClient.topic("/topic/public/${crewId}").subscribe { topicMessage ->
            Log.d(
                TAG,
                "runStomp: ${gson.fromJson(topicMessage.payload, MessageHistory::class.java)}"
            )
    ...
    fun sendMessage(messageContent: MessageData) {
        ...
        stompClient.send("/app/chat.sendMessage", gson.toJson(messageContent).toString())
            .subscribe()
    }
